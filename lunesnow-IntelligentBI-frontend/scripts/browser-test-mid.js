...
// ============================================================
(async () => {
  const TASK_DONE_MS = 30000      // 【场景参数】任务完成耗时（中任务）
  ...
  setTimeout(() => { __hidden = true; document.dispatchEvent(new Event('visibilitychange')) }, HIDDEN_START)
  setTimeout(() => { __hidden = false; document.dispatchEvent(new Event('visibilitychange')) }, HIDDEN_END)

  // 观测结束，统计对比
  setTimeout(() => {
    try { poller && poller.stop() } catch (e) {}
    ...
    console.log(`模拟隐藏时段           ：${HIDDEN_START / 1000}s ~ ${HIDDEN_END / 1000}s（共 ${(HIDDEN_END - HIDDEN_START) / 1000}s）`)
  ...
  console.log(`[测试已启动] 场景=中任务(${TASK_DONE_MS / 1000}s)，约 ${OBS_MS / 1000}s 后自动输出对比...`)
})()
